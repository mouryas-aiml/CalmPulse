...
function Insights() {
  ...
  const [readInsights, setReadInsights] = useState(() => {
  ...
  const viewInsight = (insight) => {
    ...
    // Track that this insight has been read
    if (!readInsights.includes(insight.id)) {
      const updatedReadInsights = [...readInsights, insight.id];
      setReadInsights(updatedReadInsights);
      localStorage.setItem('readInsights', JSON.stringify(updatedReadInsights));
      
      // Update user progress in Firestore
      if (currentUser) {
        updateUserProgress(currentUser.uid, 'insightsRead').catch(error => {
          console.error('Error updating insights read progress:', error);
        });
      }
    }
  };
